...
namespace Blooper.TransitionEffects.Editor
{
	...
	public class TransitionEffectPassSettingsPropertyDrawer : PropertyDrawer
	{
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			var centerRect = new Rect(position.x, transitionRect.y + transitionRect.height, position.width, EditorGUIUtility.singleLineHeight);
			...
			if ((TransitionType)tType.enumValueIndex == TransitionType.CircleWipe)
			{
				_showExtra = true;
				EditorGUI.PropertyField(centerRect, property.FindPropertyRelative("Center"), new GUIContent("Center"));
			}else if ((TransitionType)tType.enumValueIndex == TransitionType.Texture)
			{
				_showExtra = true;
		...
		{
			return EditorGUIUtility.singleLineHeight * (_showExtra ? 5 : 4);
		}
	}
}